<template>
  <div class="defi-swap-egt-page">
    <v-row justify="center" class="my-15">
      <v-col cols="11" sm="9">
        <borderWrapCard class="mb-13">
          <template #content>
            <titleBlock class="mb-7" title="EGT / ETH"></titleBlock>

            <v-form ref="form" lazy-validation class="mb-5 w-100">
              <inputBlock
                title="tokenBalance"
                mode="onlyText"
                :inputText="$t('tokenBalanceAmount', {amount: balance.toLocaleString(), token: 'EGT'})"
              ></inputBlock>
              <inputBlock title="swapAmount" :unit="$t('piece')" mode="balance" :token="'eth'" :inputText.sync="amount" :disabled="ethAllowance < ethBalance" :balance="ethBalance" :rules="[...TokenAmountRules]"></inputBlock>
            </v-form>

            <imgBtn class="mb-3" v-if="ethAllowance===0 || ethAllowance<ethBalance" dark type="bg-black-sloped" buttonText="approve" @clickBtn="approve()"></imgBtn>
            <imgBtn class="mb-3" v-else dark type="bg-black-sloped" buttonText="swapTitle" @clickBtn="swap()"></imgBtn>
  ...
  methods:{
    async approve(){
      if (this.$store.state.chainId){
        let result = await this.ercContract.approveOther(this.$store.state.EGTSwapAddress)
        // console.log('result', result)
        if (result.txHash){
          this.loadingShow = true
          this.loadingText = 'waitApprove'
          this.timer = window.setInterval(async () => {
            this.ethAllowance = await this.ercContract.getOtherAllowance(this.$store.state.account, this.$store.state.EGTSwapAddress)
            if (this.ethAllowance >= this.ethBalance) {
    ...
    async swap(){
      ...
      if (this.$store.state.chainId){
        if (this.$refs.form.validate()){
          let result = await this.egtSwapContract.buy(this.amount)
          // console.log('result', result)
          if (result.txHash){
            this.loadingShow = true
